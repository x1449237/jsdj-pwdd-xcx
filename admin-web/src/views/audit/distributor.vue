<template>
  <div class="page-container">
    <div class="page-header">
      <span class="page-title">分销商审核</span>
    </div>
    ...
    <el-card>
      <el-table :data="tableData" v-loading="loading" stripe border style="width: 100%">
        ...
        <el-table-column prop="subCount" label="下级人数" width="90" align="center" />
        <el-table-column prop="totalCommission" label="累计佣金" width="110" align="center" />
    ...
    >
      <div v-loading="detailLoading">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="下级人数">{{ detailData.subCount || 0 }}</el-descriptions-item>
          <el-descriptions-item label="累计佣金">¥{{ detailData.totalCommission || 0 }}</el-descriptions-item>
        </el-descriptions>

        <h4 class="detail-sub-title">下级分销商列表</h4>
        <el-table :data="detailData.subDistributors || []" stripe border size="small">
          <el-table-column prop="nickname" label="昵称" min-width="120" />
  ...
  methods: {
    async fetchList() {
      ...
      try {
        ...
        const res = await request.get('/admin/audit/distributors', { params })
        this.tableData = res.data?.list || []
        this.pagination.total = res.data?.total || 0
      } catch (err) {
        console.error('获取分销商审核列表失败:', err)
      } finally {
        this.loading = false
      }
    },
    ...
    async handleApprove(row) {
      try {
        await ElMessageBox.confirm(
          `确定要通过「${row.nickname}」的分销商审核吗？`,
          '审核通过确认',
          { confirmButtonText: '确定', cancelButtonText: '取消', type: 'info' }
        )
        await request.post(`/admin/audit/distributors/${row.id}/approve`)
    ...
    async handleRejectSubmit() {
      ...
      try {
        await request.post(`/admin/audit/distributors/${this.currentRejectId}/reject`, {
    ...
    async handleDetail(row) {
      ...
      try {
        const res = await request.get(`/admin/audit/distributors/${row.id}`)
        this.detailData = res.data || {}
      } catch (err) {
        console.error('获取分销商详情失败:', err)
      } finally {
    ...
    async handleDelist(row) {
      try {
        await ElMessageBox.confirm(
          `确定要强制下架分销商「${row.nickname}」吗？`,
          '强制下架确认',
          { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' }
        )
        await request.post(`/admin/audit/distributors/${row.id}/delist`)
